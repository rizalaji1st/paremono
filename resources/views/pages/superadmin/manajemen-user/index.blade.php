...
@section('menu_admin_manajemen_user_active', 'active')
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <button class="btn btn-icon btn-active-light-primary w-30px h-30px me-3" data-bs-toggle="modal" data-bs-target="#kt_modal_update_user" onclick="setDataUpdateUser({{$user->id}}, '{{$user->name}}', '{{$user->email}}', {{json_encode($user->userRoles->pluck('roles_id')->toArray())}})">
                                            <span class="svg-icon svg-icon-3">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div class="modal fade" id="kt_modal_update_user" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-dialog modal-dialog-centered mw-650px">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                            <!--begin::Form-->
                            <form id="kt_modal_update_user_form" method="POST" class="form" action="{{url('/admin/manajemen-user/update')}}">
                                @csrf
                                ...
                                <div class="fv-row mb-7">
                                    ...
                                    <label class="fs-6 fw-bold form-label mb-2">
                                        <span>Password</span>
                                        <i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="popover" data-bs-trigger="hover" data-bs-html="true" data-bs-content="enter the password to change."></i>
                                    </label>
                                    <!--end::Label-->
                                    <!--begin::Input-->
                                    <input type="password" class="form-control form-control-solid" name="password" id="update_user_password"/>
                                ...
                                <div class="mb-7">
                                    ...
                                    @foreach ($roles as $role)
                                        ...
                                        <div class="d-flex fv-row">
                                            ...
                                            <div class="form-check form-check-custom form-check-solid">
                                                <!--begin::Input-->
                                                <input class="form-check-input me-3" name="role_id" type="radio" value="{{$role->id}}" id="kt_modal_update_role_option_{{$role->id}}">
                                                <!--end::Input-->
                                                <!--begin::Label-->
...
@push('script_stack')
    <script>
        ...
        var initPermissionsList = () => {
            ...
            datatable = $(table).DataTable({
                ...
                'order': [],
                'columnDefs': [
                    { orderable: false, targets: 2 }, // Disable ordering on column 2 (assigned to)
                    { orderable: false, targets: 3 }, // Disable ordering on column 3 (action)
                ]
            });        
        }
    ...
    function setDataUpdateUser(id, name, email, roles){
        roles.forEach(role => {
            console.log(role);
            $("#kt_modal_update_role_option_"+role).prop("checked", true);
        })
                ...
                if (validator) {
                    validator.validate().then(function (status) {
                        ...
                        if (status == 'Valid') {
                            ...
                            document.getElementById('kt_modal_update_user_form').submit()
                        } else {
                            // Show popup warning. For more info check the plugin's official documentation: https://sweetalert2.github.io/
                            Swal.fire({
                                text: "Sorry, looks like there are some errors detected, please try again.",
